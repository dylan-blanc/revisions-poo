...
class Clothing extends Product
{
    ...
    public function findOneById(int $id)
    {
        ...
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($row) {
            parent::findOneById($id);
            $this->size = $row['size'] ?? '';
            $this->color = $row['color'] ?? '';
            $this->type = $row['type'] ?? '';
            $this->material_fee = isset($row['material_fee']) ? (int)$row['material_fee'] : 0;
            return $this;
        }
        return false;
    }
        ...
        foreach ($products as $product) {
            ...
            if ($row) {
                ...
                $clothings[] = new Clothing(
                    ...
                    $row['color'] ?? '',
                    $row['type'] ?? '',
                    isset($row['material_fee']) ? (int)$row['material_fee'] : 0
                );
            }
        }
        return $clothings;
...
class Electronic extends Product
{
    ...
    public function findOneById(int $id)
    {
        ...
        if ($row) {
            ...
            $this->waranty_fee = isset($row['waranty_fee']) ? (int)$row['waranty_fee'] : 0;
            return $this;
        }
        return false;
    }
    ...
    public static function findAll(PDO $pdo): array
    {
        ...
        $electronics = [];
        foreach ($products as $product) {
            $stmt = $pdo->prepare("SELECT brand, waranty_fee FROM product WHERE id = ? AND type = 'electronic'");
            $stmt->execute([$product->getId()]);
            $row = $stmt->fetch(PDO::FETCH_ASSOC);
            if ($row) {
                $pdoToUse = method_exists($product, 'getPdo') ? $product->getPdo() : $pdo;
